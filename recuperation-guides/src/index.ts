...
const recuperateurDeGuides = new RecuperateurDAdressesDesGuides(lecteurDeSiteHttp);
...
const guides = await Promise.all(
  adresses.concat(adresses_anglaises).map(async (adresse) => await recuperateurGuide.recupere(adresse))
);
