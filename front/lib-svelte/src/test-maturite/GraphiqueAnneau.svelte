<script lang="ts">
  ...
  const pourcentageVersRadian = (p: number) => (p / 100) * 2 * Math.PI - Math.PI / 2;
  ...
  $: {
    ...
    for (let index = 0; index < pourcentagesCumules.length - 1; index++) {
      ...
      if (pourcentagesCumules[index] === 0 && pourcentagesCumules[index + 1] === 100) {
      ...
      secteurs.push({
        ...
        premierPointInterieur: polaireVersCartesien(rayonInterieur, nouvelAngle),
...
<svg id="radar" viewBox="-130 -130 260 260" xmlns="http://www.w3.org/2000/svg">
  {#if nomDeLaDonnee}
    <text class="total" x="0" y="0" text-anchor="middle" font-size="40">{totalSerie(serie)}</text>
    <text class="libelle-total" x="0" y="15px" font-size="16" dominant-baseline="hanging" text-anchor="middle"
      >{nomDeLaDonnee}
    </text>
  {/if}
    ...
    <path
      ...
      d="M {secteur.premierPointExterieur.x} {secteur.premierPointExterieur.y}
        ...
        .secondPointExterieur.y}
         ...
         A {rayonInterieur} {rayonInterieur} 0 {secteur.arcLarge ? 1 : 0} 0 {secteur.secondPointInterieur.x} {secteur
        .secondPointInterieur.y}
         Z"><title>{`${serie[index].libelle} : ${valeurSerieAffichee}`}</title></path
    >
  {/each}
</svg>
