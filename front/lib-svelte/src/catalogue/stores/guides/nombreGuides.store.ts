...
export const nombreGuides = derived<[typeof guidesStore], NombreResultats>([guidesStore], ([guidesStore]) => {
  ...
  const nombreParLangue = (langue: Langue) => guidesStore.filter((guide) => guide.langue === langue).length;

  const nombreParCollection = (collection: CollectionGuide) =>
    guidesStore.filter((guide) => guide.collections.includes(collection)).length;
  return {
