...
export class EntrepotFinancementGrist implements EntrepotFinancement {
  ...
  tous = async () => {
    const urlDocFinancement = this.adaptateurEnvironnement.grist().urlFinancements();
    if (!urlDocFinancement) {
      return [];
    }
    const cleApi = this.adaptateurEnvironnement.grist().cleApiFinancements();
    const reponse = await this.clientHttp.get<RetourApiGrist>(urlDocFinancement, {
      ...
      .map(
        ({ fields }) =>
          ({
            ...
            operationsEligibles: aseptiseHtml(fields.Operations_eligibles ?? ''),
