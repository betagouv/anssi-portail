...
export const ressourceDocumentGuide = ({ busEvenements, cellar }: ConfigurationServeur) => {
  ...
  routeur.get('/:nomFichier', async (requete: Request, reponse: Response, suite: NextFunction) => {
    try {
      const documentCellar = await cellar.get(requete.params.nomFichier as string, 'GUIDES');
      if (!documentCellar) {
        reponse.sendStatus(404);
        return;
      }
      reponse.contentType(documentCellar.typeDeContenu).status(200).send(documentCellar.contenu);
