...
export class RegleFournitureDeServicesNumerique implements Regle {
  constructor(private readonly localisationsAcceptees: AppartenancePaysUnionEuropeenne[]) {}
  ...
  static nouvelle(texte: SpecificationTexte): RegleFournitureDeServicesNumerique | undefined {
    ...
    if (!tousConnus) throw new ErreurLectureDeRegle(valeur, 'Extra - Fourniture de service');

    return new RegleFournitureDeServicesNumerique(morceaux.map((m) => mapping[m]));
  }
}
