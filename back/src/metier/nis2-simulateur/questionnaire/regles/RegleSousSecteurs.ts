...
import { libellesSousSecteursActivite } from '../../LibellesSousSecteursActivite.js';
...
const chercheSousSecteurAutre = (secteurParent: string) => {
  switch (secteurParent) {
    ...
    default:
      throw new ErreurLectureDeRegle(`Autre sous-secteur pour le secteur parent ${secteurParent}`, 'Sous-secteur');
  }
};

const chercheSousSecteurPrecis = (sousSecteurAttendu: string) => {
  const sousSecteurCorrespondant = Object.entries(libellesSousSecteursActivite).find(
    ([, valeur]) => valeur == sousSecteurAttendu
  );

  if (!sousSecteurCorrespondant) throw new ErreurLectureDeRegle(sousSecteurAttendu, 'Sous-secteurs');
