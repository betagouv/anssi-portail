...
const mappingDirect: Record<string, Activite> = {
  ...
  'Infrastructure des marchés financiers': 'autreActiviteInfrastructureMarcheFinancier',
...
const mappingFabrication: Record<string, Activite> = {
  ...
  'Fabrication de machines et équipements n.c.a.': 'autreActiviteFabricationMachinesEquipements',
  "Fabrication d'équipements électriques": 'autreActiviteFabricationEquipementsElectroniques',
  "Fabrication d'autres matériels de transport": 'autreActiviteConstructionVehiculesAutomobilesRemorquesSemi',
};
...
const recupereActiviteIdentifiee = (valeur: string) => {
  const activiteCorrespondante = Object.entries(libellesActivites).find(([, libelle]) => libelle === valeur);

  if (!activiteCorrespondante) throw new ErreurLectureDeRegle(valeur, 'Activités');
...
const recupereReseauTransport = (texte: SpecificationTexte) => {
  ...
  if (sousSecteur === 'Électricité') return new RegleActivites('gestionnaireReseauTransportElectricite');
  if (sousSecteur === 'Gaz') return new RegleActivites('gestionnaireReseauTransportGaz');
