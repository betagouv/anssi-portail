...
export class EnsembleDeSpecifications {
  ...
  evalue(reponses: EtatQuestionnaire): ResultatAvecAnalyse {
    const passants = this.specifications.filter((s) => s.evalue(reponses) !== undefined);

    if (passants.length === 0) {
      const detail = JSON.stringify(reponses);
      throw new Error(`Aucune spécification ne correspond au questionnaire. ${detail}`);
    }
...
function choisisLeResultat(passants: Specifications[]) {
  ...
  const tousLesResumes = passants.flatMap((p) => p.resultat().pointsAttention.resumes);
  const toutesPrecisions = passants.flatMap((p) => p.resultat().pointsAttention.precisions);
  ...
  return {
    ...resultatPlusStrict,
    pointsAttention: { resumes: resumesUniques, precisions: precisionsUniques },
  };
}

function prioriseLesSpecifications(a: Specifications, b: Specifications): number {
