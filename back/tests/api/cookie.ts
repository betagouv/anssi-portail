...
const decodeSessionDuCookie = (reponse: Response, indiceHeader: number) => {
  try {
    ...
    return JSON.parse(Buffer.from(cookieSession.session as string, 'base64').toString());
  } catch {
    return undefined;
  }
};
...
const encodeSession = (contenuSession: Session) => {
  const sessionEnBase64 = Buffer.from(JSON.stringify(contenuSession), 'utf-8').toString('base64');
