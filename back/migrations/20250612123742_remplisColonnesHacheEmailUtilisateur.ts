  ...
  await knex.transaction(async (trx) => {
    ...
    const maj = utilisateurs.map(({ email }) => {
      ...
      return trx('utilisateurs')
        .where({ email })
        .update({ email_hache: emailHacheHMAC, email_hache_256: emailHache256 });
    });
...
export async function down(knex: Knex): Promise<void> {
  await knex.transaction(async (trx) => trx('utilisateurs').update({ email_hache: null, email_hache_256: null }));
}
