...
>
    ...
    <polygon points={tableauEnPointsPolygone(pointsDuPolygone)} fill="#FED98099" stroke="#0D0C21" stroke-width="3"/>
    {#each new Array(6).fill(0).map((_, index) => index) as index}
        {#if index !== 0 }
            {@const r = (tailleRadar / 5 * index)}
            {@const y = -r * Math.cos(-Math.PI / 6)}
            <text x="0" y={y} fill="#0D0C21" text-anchor="middle" font-size="15" stroke="white" stroke-width="0.6"
